<script lang="ts">
	...
	const toggleValue = (option: { label: string; value: string }) => {
...
<div class={className} use:clickOutside on:clickOutside={() => (isOpen = false)}>
	...
	<div class="relative mt-2">
		...
		>
			<span class={`block truncate ${!value.length ? 'text-gray-400' : ''}`}
				>{options.find((o) => value.includes(o.value))?.label
					? `${options.find((o) => value.includes(o.value))?.label} ${
							options.filter((o) => value.includes(o.value)).length > 1
								? `+${options.filter((o) => value.includes(o.value)).length - 1}`
								: ''
						}`
					: placeholder}</span
			...
			>
				...
				{#each showedOptions as option}
					<li
						...
						on:keypress={() => toggleValue(option)}
						on:click={() => toggleValue(option)}
					>
						<span
							class={`${
								value.includes(option.value) ? 'font-semibold' : 'font-normal'
							} block truncate`}>{option.label}</span
						>
